...
// Agregar personas a la BD, solo 1 vez para que no se repitan en firebase
/*const personas = [
...
db.collection('usuarios').orderBy('numero', 'asc').limit(4).onSnapshot((snapshot) => { //collection es la coleccion de usuarios que hay en firestore
    //console.log(snapshot.docs[0].data());
    cargarDocumentos(snapshot.docs);
});

const cargarDocumentos = (documentos) => {
    if (documentos.length > 0) { //si hay documentos los muestro
...
botonAnterior.addEventListener('click', () => {
    db.collection('usuarios').orderBy('numero', 'desc').limit(4).startAfter(primerDoc).onSnapshot((snapshot) => {
        const documentos = snapshot.docs.reverse(); //reverse() para que no aparesca el orden de forma invertida
